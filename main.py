...
def addJobs():
    try:
        ...
        if request.method == 'POST':
            email = session['email']
            insertjobapplication = InsertJobApplication.InsertJobApplication(email,'')
            message = insertjobapplication.insertJobApplication()
...
def addJobApplication():
    try:
        if request.method == 'POST':
            ...
            jobData = fetchjobdata.getJobData_BSL()
            insertjobapplication = InsertJobApplication.InsertJobApplication(email,'')
            applicationMessage = insertjobapplication.insertJobApplication()
            noOfJobs = len(jobData)
            relesEngine = Businesslayer_RulesEngine.Businesslayer_RulesEngine()
            allow = relesEngine.rulesEngine_Employer_BSL(myUser.email,myUser.userType,myUser.planType)
            return render_template("jobs.html", application_msg = applicationMessage,userType=userType,jobData=jobData,noOfJobs=noOfJobs,allow=allow)
    except Exception as e:
